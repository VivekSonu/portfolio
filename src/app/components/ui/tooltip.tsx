...
const Tooltip: FC<TooltipProps>=({title,image,bgColor})=>{
    ...
    return (
        ...
        {/*Title*/}
        {active ? <div className="absolute -top-6 bg-black/[0.2] py-0.5 px-1.5 rounded-2xl backdrop-filter transition-all duration-200">
          <div className="font-pixel text-[10px] whitespace-nowrap">{title}</div>
        </div>:null}
    </div>
    );
}
